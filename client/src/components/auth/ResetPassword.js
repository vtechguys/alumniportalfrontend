...
class ResetPassword extends Component {
  ...
  callAPI = (userData = {}) => {
    ...
    axios
      .post(`${BASE_URL}/api/users/reset-password/${email}/${hash}`, userData)
      .then(res =>
        this.props.dispatch({
          type: RESET_PASSWORD
        })
  ...
  render() {
    ...
    if (currTimestamp - timestamp > 5 * 60 * 1000) {
      // 5 min window
      this.callAPI();
      return <div className="alert alert-secondary">Link Expired !!!</div>;
    } else {
    }
    return (
      <div>
